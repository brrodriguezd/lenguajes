...
struct ExecEnviron {
  int x; /* The value of the x variable, a real language would have some
            name->value lookup table instead */
};
...
static int (*valExecs[])(struct ExecEnviron *e, struct AstElement *a) = {
    execTermExpression, execTermExpression, execBinExp, NULL, NULL, NULL, NULL};
...
static void (*runExecs[])(struct ExecEnviron *e, struct AstElement *a) = {
    ...
    execAssign, execWhile, execCall, execStmt,
};
...
static void onlyName(const char *name, const char *reference,
                     const char *kind) {
...
static void onlyX(const char *name) { onlyName(name, "x", "variable"); }

static void onlyPrint(const char *name) { onlyName(name, "print", "function"); }
...
void freeEnv(struct ExecEnviron *e) { free(e); }